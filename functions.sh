...
create_sgs() {
  ...
  for sg_sulfix in "${security_group_sulfixes[@]}"; do
    ...
    --group-id $sg_id --protocol http --port 80 
    fi
  done
}
...
validate_env_file() {
  ...
  if [ $has_repository == true ]; then
    ...
    if [ $has_repository_branch == true ]; then
      ...
      if [[ $repository_branch_value = \"production\" ]] || [[ $repository_branch_value = \"staging\" ]] || [[ $repository_branch_value = \"development\" ]] || [[ $repository_branch_value = \"prod\" ]] || [[ $repository_branch_value = \"stag\" ]] || [[ $repository_branch_value = \"dev\" ]] || [[ $repository_branch_value = \"qa\" ]]; then
