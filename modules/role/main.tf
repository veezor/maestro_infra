...
resource "aws_iam_role" "codebuild_role" {
  name               = format("%s-%s-%s-CodeBuild-Maestro", "${var.owner}", "${var.project_name}", "${var.environment}")
...
resource "aws_iam_role" "ecs_role" {
  name               = format("%s-%s-%s-ECS-Maestro", "${var.owner}", "${var.project_name}", "${var.environment}")
  assume_role_policy = <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
